...
class SMILESParser(object):
    ...
    def parse_smiles(self, smiles_array, targets_array=None, ids_array=None, verbose=False):
        ...
        for i in range(n_smiles):
            if not verbose:
                print(f"\r\t{i+1}/{n_smiles}", end='', flush=True)
            ...
            # Otherwise, report the error (if verbose is enabled)
                ...
                print(f"\tParsing failed for SMILES string {i + 1}/{n_smiles}, with value {smiles_array[i]}...",
                      end='', flush=True)
    ...
    def parse_smiles_str(self, smiles_str, id, target=None):
        ...
        # Represent Hydrogen atoms explicitly (if necessary)
    ...
    def get_num_node_features(self):
        # Initialize number of node features to zero
        num_node_features = 0
        ...
        num_node_features += self.count_num_features(self.config['atom']['degree'], MAX_ATOM_DEGREE + 1,
                                                     "atom degree")
        # Number of features used to encode the number of implicit hydrogen atoms
        num_node_features += self.count_num_features(self.config['atom']['num_Hs'], MAX_ATOM_NUM_HS + 1,
                                                     "atom num Hs")
    ...
    def get_num_edge_features(self):
        # Initialize number of edge features to zero
        num_edge_features = 0

        # Number of features used to encode the bond type
        num_edge_features += self.count_num_features(self.config['bond']['type'], NUM_ALLOWABLE_BOND_TYPES,
                                                     "bond type")
        # Number of features used to encode whether the bond is conjugated or not
        num_edge_features += self.count_num_features(self.config['bond']['is_conjugated'], 2,
                                                     "bond is conjugated")
        # Number of features used to encode whether the bond is in a ring or not
        num_edge_features += self.count_num_features(self.config['bond']['is_in_ring'], 2,
                                                     "bond is in ring")

        return num_edge_features

    def get_node_features(self, atom):
        # Initialize atom feature vector as an empty list
        node_features = []
        ...
        # Append feature vector corresponding to the implicit valence of the atom to the list of node-specific features
        ...
        # Check that the feature vector is not empty
    ...
    def get_edge_features(self, bond):
        # Initialize bond feature vector as an empty list
        edge_features = []
    ...
    def encode_feature(mode, val, n_vals, feat_name):
        ...
        # Sanity-check
        if val > n_vals:
            raise ValueError(f"{feat_name}: {val}. Maximum {feat_name} recognised by the model: {n_vals - 1}.")
        ...
        # Unrecognised option
            ...
            raise ValueError(f"Unrecognised encoding format {mode} for {feat_name}")
    ...
    def count_num_features(mode, n_vals, feat_name):
        ...
        # Unrecognized option
            ...
            raise ValueError(f"Unrecognized encoding format {mode} for {feat_name}.")
